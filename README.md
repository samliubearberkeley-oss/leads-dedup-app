...
- 系统会自动切换到"新Leads"标签页
- 在"新Leads"标签页可以查看所有未发送的联系人
- 表格显示：YouTube Channel、名字/用户名、邮箱
- 可以导出CSV或复制全部邮箱

### 4. 保存到数据库

- **重要**：上传CSV后，系统会先展示去重结果，不会自动保存
- 在"新Leads"标签页确认数据无误后，点击"保存到数据库"按钮
- 保存完成后会自动跳转到"已发送"列表查看结果
- 下次上传相同数据时会被自动过滤为重复记录
